...
sap.designstudio.sdk.PropertyPage.subclass("org.scn.community.generic.PropertyPage", function () {
						...
						var changeHandler = function (property, handler) {
							return function (oControlEvent) {
								...
								if (!that.isTest) {
									...
									// [vanqm] Update the property page if 'swapAxes' changes.
									if(property == "swapAxes") {
										that.updatePropertyPage();
									}
								} else {
									alert("Property: " + property + "\r\nValue:\r\n" + newValue);
								}
	...
	/* [vanqm]
	 * Update the value of TextFields, ... in PropertyPage.html
	 * because the data changes
	 * */
	this.updatePropertyPage = function() {
		// Should delay to data be updated
		setTimeout(function(){ 
			var properties_newvalues = JSON.parse(that.callRuntimeHandler("propertyNewValueList"));
			for(var i = 0; i < properties_newvalues.length; i++) {
				that["cmp_" + properties_newvalues[i].key].setValue(properties_newvalues[i].value);
			}
		}, 500);
	};
	...
	this.init = function () {
		...
		try {
			...
			this.updateDataInfo = function (reason) {
				...
				// [vanqm] DATA_SOURCE_ALIAS_REF changed, we also should update the property page.
				that.updatePropertyPage();
